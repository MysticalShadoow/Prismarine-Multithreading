...
function bloodhound(bot) {
    ...
    function cleanUsedEvents(events) {
        for (let i = events.length - 1; i >= 0; i--) {
            if (events[i].used) {
                events.splice(i, 1);
            }
    ...
    function correlateAttack(hurtIndex, attackIndex) {
        const hurt = lastHurts[hurtIndex];
        const attack = lastAttacks[attackIndex];
        ...
        if (meleeDist > maxMeleeDist)
            return;
        const weapon = attack.entity.heldItem;
        if (bot.bloodhound.yawCorrelation === true &&
    ...
    function correlateAttacks() {
        if (lastHurts.length > maxEventsSizeCleanup)
            cleanupEvents(lastHurts);
        if (lastAttacks.length > maxEventsSizeCleanup)
            cleanupEvents(lastAttacks);
        if (lastHurts.length === 0 || lastAttacks.length === 0)
            return;
        for (let hurtIndex = 0; hurtIndex < lastHurts.length; hurtIndex++) {
            if (lastHurts[hurtIndex].used)
                continue;
            for (let attackIndex = 0; attackIndex < lastAttacks.length; attackIndex++) {
                if (lastAttacks[attackIndex].used)
                    continue;
    ...
    function associateProjectile(entity) {
    ...
    function updateProjectile(entity) {
    ...
    function detectShooter(hurtEntity, entity) {
        ...
        if (entity.type !== 'projectile' || distance > 3.5)
            return;
        const projectile = shotProjectiles.get(entity.id);
        if (!projectile)
            return;
    ...
    function detectTrident(hurtEntity) {
    ...
    bot.on('entityGone', (entity) => {
        if (!bot.bloodhound.projectileDetection)
            return;
        else if (entity.type !== 'projectile')
            return;
        if (damageEventTriggered && hurtEntity)
            void detectShooter(hurtEntity, entity);
    });
