 ...
 */
class Shot {
    constructor(originVel, { position: pPos, velocity: pVel, gravity, airResistance }, interceptCalcs) {
        this.blockHit = false;
        this.blockCheck = false;
        const { yaw, pitch } = (0, mathUtils_1.dirToYawAndPitch)(pVel);
    ...
    hitsEntityWithPrediction({ position, height, width }, avgSpeed) {
        ...
        const { newTarget } = (0, mathUtils_1.getPremonition)(this.initialPos, position.clone().add(avgSpeed.clone().scale(calcShot.totalTicks + 5)), avgSpeed, calcShot.totalTicks);
    ...
    calcToIntercept(blockChecking = false, entities = []) {
        ...
        let offsetX = -currentVelocity.x * this.airResistance;
        let offsetY = -currentVelocity.y * this.airResistance - gravity;
        let offsetZ = -currentVelocity.z * this.airResistance;
